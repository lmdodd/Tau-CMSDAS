...
int main(int argc, char** argv) {
	...
	for (Int_t i = 0; i < nentries_wtn; i++) {
		...
		if (nMu<=1){
			...
			bool Select_Tau_event= ((HLTEleMuX >> 29 & 1) == 1 || (HLTEleMuX >> 30 & 1) == 1 ) && muPt->at(imu) > 25 && fabs(muEta->at(imu)) < 2.1 && (muIsMediumID->at(imu) > 0) && IsoMu < 0.10 && fabs(muD0->at(imu)) < 0.045 && fabs(muDz->at(imu)) < 0.2 && MuMetTranverseMass <40 && nEle==0; //Ztautau selection Ask for single Iso and in MT range we expect.

			if (!Select_Tau_event) continue;
